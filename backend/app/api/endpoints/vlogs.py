...
async def create_vlog(vlog_in: VlogCreate, db: AsyncSession = Depends(get_db)):
    """
...
async def get_vlogs(db: AsyncSession = Depends(get_db)):
    """
    Retrieve all vlogs, ordered by newest first.
    """
...
async def delete_vlog(vlog_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    """
    Delete a vlog entry.
    """
